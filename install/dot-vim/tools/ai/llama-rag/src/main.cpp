// src/main.cpp
#include <filesystem>
#include <fstream>
#include <iostream>
#include <optional>
#include <string>
#include <vector>
#include <cmath>
#include <cstdlib>
#include <algorithm>
#include <cctype>
#include <unordered_set>

#include <sqlite3.h>

#include "httplib.h"
#include <nlohmann/json.hpp>

namespace fs = std::filesystem;
using json = nlohmann::json;

struct RetrievedChunk {
    std::string path;
    std::string text;
    float sem_score;
    float score;
};

std::string read_text_file(const fs::path& p) {
    std::ifstream ifs(p, std::ios::binary);
    if (!ifs) return {};
    return std::string((std::istreambuf_iterator<char>(ifs)), std::istreambuf_iterator<char>());
}

std::vector<std::string> chunk_text(const std::string& s, size_t chunk = 1000, size_t overlap = 200) {
    std::vector<std::string> out;
    if (s.empty()) return out;
    size_t start = 0;
    while (start < s.size()) {
        size_t end = std::min(start + chunk, s.size());
        out.emplace_back(s.substr(start, end - start));
        if (end == s.size()) break;
        start = end - std::min(overlap, end);
    }
    return out;
}

std::string to_valid_utf8(const std::string& in) {
    std::string out;
    out.reserve(in.size());

    size_t i = 0;
    while (i < in.size()) {
        unsigned char c = static_cast<unsigned char>(in[i]);

        if (c <= 0x7F) {
            out.push_back(static_cast<char>(c));
            ++i;
            continue;
        }

        if (c >= 0xC2 && c <= 0xDF) {
            if (i + 1 < in.size()) {
                unsigned char c1 = static_cast<unsigned char>(in[i + 1]);
                if ((c1 & 0xC0) == 0x80) {
                    out.push_back(static_cast<char>(c));
                    out.push_back(static_cast<char>(c1));
                    i += 2;
                    continue;
                }
            }
            out.push_back('?');
            ++i;
            continue;
        }

        if (c >= 0xE0 && c <= 0xEF) {
            if (i + 2 < in.size()) {
                unsigned char c1 = static_cast<unsigned char>(in[i + 1]);
                unsigned char c2 = static_cast<unsigned char>(in[i + 2]);
                bool ok = ((c1 & 0xC0) == 0x80) && ((c2 & 0xC0) == 0x80);
                if (c == 0xE0) ok = ok && (c1 >= 0xA0);
                if (c == 0xED) ok = ok && (c1 <= 0x9F);
                if (ok) {
                    out.push_back(static_cast<char>(c));
                    out.push_back(static_cast<char>(c1));
                    out.push_back(static_cast<char>(c2));
                    i += 3;
                    continue;
                }
            }
            out.push_back('?');
            ++i;
            continue;
        }

        if (c >= 0xF0 && c <= 0xF4) {
            if (i + 3 < in.size()) {
                unsigned char c1 = static_cast<unsigned char>(in[i + 1]);
                unsigned char c2 = static_cast<unsigned char>(in[i + 2]);
                unsigned char c3 = static_cast<unsigned char>(in[i + 3]);
                bool ok = ((c1 & 0xC0) == 0x80) && ((c2 & 0xC0) == 0x80) && ((c3 & 0xC0) == 0x80);
                if (c == 0xF0) ok = ok && (c1 >= 0x90);
                if (c == 0xF4) ok = ok && (c1 <= 0x8F);
                if (ok) {
                    out.push_back(static_cast<char>(c));
                    out.push_back(static_cast<char>(c1));
                    out.push_back(static_cast<char>(c2));
                    out.push_back(static_cast<char>(c3));
                    i += 4;
                    continue;
                }
            }
            out.push_back('?');
            ++i;
            continue;
        }

        out.push_back('?');
        ++i;
    }

    return out;
}

std::optional<std::vector<float>> embed(const std::string& base_url, const std::string& text) {
    httplib::Client cli(base_url.c_str());
    cli.set_connection_timeout(2, 0);
    cli.set_read_timeout(30, 0);

    std::string safe_text = to_valid_utf8(text);
    if (safe_text.empty()) return std::nullopt;

    for (int attempt = 0; attempt < 4; ++attempt) {
        json req_openai = { {"input", safe_text} };
        auto res = cli.Post("/v1/embeddings", req_openai.dump(), "application/json");
        if (res && res->status == 200) {
            try {
                json j = json::parse(res->body);
                return j.at("data").at(0).at("embedding").get<std::vector<float>>();
            } catch (...) {
                std::cerr << "Unexpected /v1/embeddings JSON shape\n";
            }
        } else if (res) {
            bool too_large = (res->status == 500 && res->body.find("input is too large") != std::string::npos);
            if (too_large && safe_text.size() > 160) {
                safe_text.resize(safe_text.size() / 2);
                continue;
            }
            std::cerr << "/v1/embeddings failed status=" << res->status;
            if (!res->body.empty()) std::cerr << " body=" << res->body.substr(0, 220);
            std::cerr << "\n";
        }

        json req_legacy = { {"content", safe_text} };
        res = cli.Post("/embedding", req_legacy.dump(), "application/json");
        if (!res || res->status != 200) {
            std::cerr << "Embedding HTTP error";
            if (res) std::cerr << " status=" << res->status;
            std::cerr << "\n";
            return std::nullopt;
        }

        try {
            json j = json::parse(res->body);
            if (j.is_array() && !j.empty()) {
                const auto& embNode = j.at(0).at("embedding");
                if (embNode.is_array() && !embNode.empty() && embNode.at(0).is_array()) {
                    return embNode.at(0).get<std::vector<float>>();
                }
                if (embNode.is_array()) {
                    return embNode.get<std::vector<float>>();
                }
            }
            std::cerr << "Unexpected embedding JSON shape\n";
        } catch (...) {
            std::cerr << "Embedding response is not valid JSON\n";
        }
        return std::nullopt;
    }

    std::cerr << "Embedding input remained too large after retries\n";
    return std::nullopt;
}

float cosine(const std::vector<float>& a, const std::vector<float>& b) {
    if (a.size() != b.size() || a.empty()) return 0.f;
    double dot = 0, na = 0, nb = 0;
    for (size_t i = 0; i < a.size(); ++i) {
        dot += a[i] * b[i];
        na += a[i] * a[i];
        nb += b[i] * b[i];
    }
    if (na == 0 || nb == 0) return 0.f;
    return static_cast<float>(dot / (std::sqrt(na) * std::sqrt(nb)));
}

std::string lower_ascii(std::string s) {
    for (char& c : s) c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
    return s;
}

std::vector<std::string> tokenize_words(const std::string& s) {
    std::vector<std::string> out;
    std::string cur;
    const std::string low = lower_ascii(s);
    for (char c : low) {
        if (std::isalnum(static_cast<unsigned char>(c))) {
            cur.push_back(c);
        } else if (!cur.empty()) {
            if (cur.size() >= 3) out.push_back(cur);
            cur.clear();
        }
    }
    if (!cur.empty() && cur.size() >= 3) out.push_back(cur);
    return out;
}

float lexical_overlap_score(const std::string& question, const std::string& text) {
    static const std::unordered_set<std::string> stop = {
        "the","and","for","with","from","that","this","what","does","tell","tells","about","who","you","your"
    };

    auto qtok = tokenize_words(question);
    auto ttok = tokenize_words(text);
    if (qtok.empty() || ttok.empty()) return 0.f;

    std::unordered_set<std::string> tset(ttok.begin(), ttok.end());
    size_t useful = 0;
    size_t matched = 0;
    for (const auto& token : qtok) {
        if (stop.find(token) != stop.end()) continue;
        ++useful;
        if (tset.find(token) != tset.end()) ++matched;
    }
    if (useful == 0) return 0.f;
    return static_cast<float>(matched) / static_cast<float>(useful);
}

float all_terms_match_bonus(const std::string& question, const std::string& text) {
    static const std::unordered_set<std::string> stop = {
        "the","and","for","with","from","that","this","what","does","tell","tells","about","who","you","your"
    };

    auto qtok = tokenize_words(question);
    auto ttok = tokenize_words(text);
    if (qtok.empty() || ttok.empty()) return 0.f;

    std::unordered_set<std::string> tset(ttok.begin(), ttok.end());
    size_t useful = 0;
    for (const auto& token : qtok) {
        if (stop.find(token) != stop.end()) continue;
        ++useful;
        if (tset.find(token) == tset.end()) return 0.f;
    }
    if (useful >= 2) return 0.25f;
    return 0.f;
}

bool parse_port(const std::string& s, int& out_port) {
    char* end = nullptr;
    long v = std::strtol(s.c_str(), &end, 10);
    if (!end || *end != '\0') return false;
    if (v < 1 || v > 65535) return false;
    out_port = static_cast<int>(v);
    return true;
}

bool parse_size_value(const std::string& s, size_t& out_value) {
    char* end = nullptr;
    unsigned long long v = std::strtoull(s.c_str(), &end, 10);
    if (!end || *end != '\0') return false;
    if (v == 0) return false;
    out_value = static_cast<size_t>(v);
    return true;
}

std::int64_t file_mtime_key(const fs::path& p) {
    try {
        return static_cast<std::int64_t>(fs::last_write_time(p).time_since_epoch().count());
    } catch (...) {
        return 0;
    }
}

std::vector<fs::path> collect_doc_files(const std::string& folder) {
    static const std::unordered_set<std::string> ignored_dir_names = {
        ".git", ".svn", ".hg", "node_modules", "build", "dist", "target", "__pycache__", ".venv", "venv"
    };

    std::vector<fs::path> files;
    for (auto it = fs::recursive_directory_iterator(folder); it != fs::recursive_directory_iterator(); ++it) {
        const auto& p = *it;

        if (p.is_directory()) {
            auto name = p.path().filename().string();
            if (!name.empty() && name[0] == '.') {
                it.disable_recursion_pending();
                continue;
            }
            if (ignored_dir_names.find(name) != ignored_dir_names.end()) {
                it.disable_recursion_pending();
                continue;
            }
            continue;
        }

        if (!p.is_regular_file()) continue;
        auto ext = p.path().extension().string();
        if (ext != ".txt" && ext != ".md") continue;

        std::uintmax_t sz = 0;
        try { sz = fs::file_size(p.path()); } catch (...) { continue; }
        if (sz > 256 * 1024) continue;

        files.push_back(p.path());
    }
    std::sort(files.begin(), files.end());
    return files;
}

bool sqlite_exec(sqlite3* db, const char* sql) {
    char* err = nullptr;
    int rc = sqlite3_exec(db, sql, nullptr, nullptr, &err);
    if (rc != SQLITE_OK) {
        std::cerr << "SQLite error: " << (err ? err : "unknown") << "\n";
        sqlite3_free(err);
        return false;
    }
    return true;
}

bool try_load_sqlite_vec(sqlite3* db) {
    sqlite3_enable_load_extension(db, 1);
    const std::vector<std::string> candidates = {
        "sqlite_vec", "sqlite-vec", "vec0", "./sqlite_vec", "./vec0"
    };

    for (const auto& c : candidates) {
        char* err = nullptr;
        if (sqlite3_load_extension(db, c.c_str(), nullptr, &err) == SQLITE_OK) {
            return true;
        }
        if (err) sqlite3_free(err);
    }
    return false;
}

bool init_db(sqlite3* db) {
    return sqlite_exec(db,
        "CREATE TABLE IF NOT EXISTS chunks ("
        "chunk_id TEXT PRIMARY KEY,"
        "path TEXT NOT NULL,"
        "text TEXT NOT NULL,"
        "embedding TEXT NOT NULL,"
        "source_sig TEXT NOT NULL"
        ");"
        "CREATE INDEX IF NOT EXISTS idx_chunks_path ON chunks(path);");
}

std::string vec_to_json_text(const std::vector<float>& v) {
    return json(v).dump();
}

std::optional<std::vector<float>> vec_from_json_text(const std::string& s) {
    try {
        return json::parse(s).get<std::vector<float>>();
    } catch (...) {
        return std::nullopt;
    }
}

bool upsert_chunk(sqlite3* db,
                  const std::string& chunk_id,
                  const std::string& path,
                  const std::string& text,
                  const std::string& embedding,
                  const std::string& source_sig) {
    const char* sql =
        "INSERT INTO chunks(chunk_id,path,text,embedding,source_sig) VALUES(?1,?2,?3,?4,?5)"
        " ON CONFLICT(chunk_id) DO UPDATE SET"
        " path=excluded.path,text=excluded.text,embedding=excluded.embedding,source_sig=excluded.source_sig;";

    sqlite3_stmt* st = nullptr;
    if (sqlite3_prepare_v2(db, sql, -1, &st, nullptr) != SQLITE_OK) return false;
    sqlite3_bind_text(st, 1, chunk_id.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(st, 2, path.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(st, 3, text.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(st, 4, embedding.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(st, 5, source_sig.c_str(), -1, SQLITE_TRANSIENT);
    bool ok = sqlite3_step(st) == SQLITE_DONE;
    sqlite3_finalize(st);
    return ok;
}

std::optional<std::string> get_chunk_signature(sqlite3* db, const std::string& chunk_id) {
    const char* sql = "SELECT source_sig FROM chunks WHERE chunk_id=?1";
    sqlite3_stmt* st = nullptr;
    if (sqlite3_prepare_v2(db, sql, -1, &st, nullptr) != SQLITE_OK) return std::nullopt;
    sqlite3_bind_text(st, 1, chunk_id.c_str(), -1, SQLITE_TRANSIENT);
    std::optional<std::string> out;
    if (sqlite3_step(st) == SQLITE_ROW) {
        const unsigned char* s = sqlite3_column_text(st, 0);
        if (s) out = reinterpret_cast<const char*>(s);
    }
    sqlite3_finalize(st);
    return out;
}

size_t count_chunks(sqlite3* db) {
    const char* sql = "SELECT COUNT(*) FROM chunks";
    sqlite3_stmt* st = nullptr;
    if (sqlite3_prepare_v2(db, sql, -1, &st, nullptr) != SQLITE_OK) return 0;
    size_t n = 0;
    if (sqlite3_step(st) == SQLITE_ROW) n = static_cast<size_t>(sqlite3_column_int64(st, 0));
    sqlite3_finalize(st);
    return n;
}

bool clear_chunks(sqlite3* db) {
    return sqlite_exec(db, "DELETE FROM chunks;");
}

bool mark_seen(sqlite3* db, const std::string& chunk_id) {
    const char* sql = "INSERT OR IGNORE INTO seen_chunks(chunk_id) VALUES(?1)";
    sqlite3_stmt* st = nullptr;
    if (sqlite3_prepare_v2(db, sql, -1, &st, nullptr) != SQLITE_OK) return false;
    sqlite3_bind_text(st, 1, chunk_id.c_str(), -1, SQLITE_TRANSIENT);
    bool ok = sqlite3_step(st) == SQLITE_DONE;
    sqlite3_finalize(st);
    return ok;
}

bool prune_unseen(sqlite3* db) {
    return sqlite_exec(db,
        "DELETE FROM chunks WHERE chunk_id NOT IN (SELECT chunk_id FROM seen_chunks);"
        "DROP TABLE seen_chunks;");
}

std::vector<RetrievedChunk> retrieve_with_vec(sqlite3* db,
                                              const std::string& qvec_json,
                                              const std::string& question,
                                              size_t candidate_k,
                                              size_t top_k,
                                              bool& used_vec) {
    used_vec = false;
    std::vector<RetrievedChunk> rows;

    const char* sql =
        "SELECT path, text, vec_distance_cosine(vec_f32(embedding), vec_f32(?1)) AS dist "
        "FROM chunks ORDER BY dist ASC LIMIT ?2";

    sqlite3_stmt* st = nullptr;
    if (sqlite3_prepare_v2(db, sql, -1, &st, nullptr) != SQLITE_OK) {
        return rows;
    }

    sqlite3_bind_text(st, 1, qvec_json.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_bind_int(st, 2, static_cast<int>(candidate_k));

    while (sqlite3_step(st) == SQLITE_ROW) {
        const unsigned char* p = sqlite3_column_text(st, 0);
        const unsigned char* t = sqlite3_column_text(st, 1);
        float dist = static_cast<float>(sqlite3_column_double(st, 2));
        if (!p || !t) continue;

        RetrievedChunk r;
        r.path = reinterpret_cast<const char*>(p);
        r.text = reinterpret_cast<const char*>(t);
        r.sem_score = 1.0f - dist;
        float s_lex = lexical_overlap_score(question, r.text);
        float s_bonus = all_terms_match_bonus(question, r.text);
        r.score = 0.65f * r.sem_score + 0.35f * s_lex + s_bonus;
        rows.push_back(std::move(r));
    }
    sqlite3_finalize(st);

    if (!rows.empty()) {
        used_vec = true;
        std::partial_sort(rows.begin(), rows.begin() + std::min(top_k, rows.size()), rows.end(),
                          [](const RetrievedChunk& a, const RetrievedChunk& b) { return a.score > b.score; });
        if (rows.size() > top_k) rows.resize(top_k);
    }

    return rows;
}

std::vector<RetrievedChunk> retrieve_fallback(sqlite3* db,
                                              const std::vector<float>& qvec,
                                              const std::string& question,
                                              size_t top_k) {
    std::vector<RetrievedChunk> rows;

    const char* sql = "SELECT path,text,embedding FROM chunks";
    sqlite3_stmt* st = nullptr;
    if (sqlite3_prepare_v2(db, sql, -1, &st, nullptr) != SQLITE_OK) return rows;

    while (sqlite3_step(st) == SQLITE_ROW) {
        const unsigned char* p = sqlite3_column_text(st, 0);
        const unsigned char* t = sqlite3_column_text(st, 1);
        const unsigned char* e = sqlite3_column_text(st, 2);
        if (!p || !t || !e) continue;

        auto ev = vec_from_json_text(reinterpret_cast<const char*>(e));
        if (!ev) continue;

        RetrievedChunk r;
        r.path = reinterpret_cast<const char*>(p);
        r.text = reinterpret_cast<const char*>(t);
        r.sem_score = cosine(qvec, *ev);
        float s_lex = lexical_overlap_score(question, r.text);
        float s_bonus = all_terms_match_bonus(question, r.text);
        r.score = 0.65f * r.sem_score + 0.35f * s_lex + s_bonus;
        rows.push_back(std::move(r));
    }
    sqlite3_finalize(st);

    std::partial_sort(rows.begin(), rows.begin() + std::min(top_k, rows.size()), rows.end(),
                      [](const RetrievedChunk& a, const RetrievedChunk& b) { return a.score > b.score; });
    if (rows.size() > top_k) rows.resize(top_k);
    return rows;
}

std::optional<std::string> chat(const std::string& base_url, const std::string& system, const std::string& user) {
    httplib::Client cli(base_url.c_str());
    cli.set_connection_timeout(2, 0);
    cli.set_read_timeout(120, 0);

    json req = {
        {"model", "local-llm"},
        {"messages", json::array({
            json{{"role","system"},{"content",system}},
            json{{"role","user"},{"content",user}}
        })},
        {"temperature", 0.2},
        {"max_tokens", 512}
    };

    auto res = cli.Post("/v1/chat/completions", req.dump(), "application/json");
    if (!res || res->status != 200) {
        std::cerr << "Chat HTTP error\n";
        return std::nullopt;
    }

    try {
        json j = json::parse(res->body);
        std::string out = j["choices"][0]["message"]["content"].get<std::string>();
        if (out.size() >= 3 &&
            static_cast<unsigned char>(out[0]) == 0xEF &&
            static_cast<unsigned char>(out[1]) == 0xBB &&
            static_cast<unsigned char>(out[2]) == 0xBF) {
            out.erase(0, 3);
        }
        while (!out.empty()) {
            unsigned char c = static_cast<unsigned char>(out.front());
            if ((c < 0x20 && c != '\n' && c != '\t') || c == 0x7F) {
                out.erase(out.begin());
                continue;
            }
            break;
        }
        return out;
    } catch (...) {
        return std::nullopt;
    }
}

int main(int argc, char** argv) {
    if (argc < 3) {
        std::cerr << "Usage: local_rag [--emb-port <port>] [--chat-port <port>] [--profile <fast|balanced|accurate>] [--top-k <k>] [--max-context-chars <n>] [--reindex] [--db-path <path>] <folder> <question>\n";
        return 1;
    }

    int emb_port = 8013;
    int chat_port = 8012;
    size_t top_k = 4;
    size_t max_context_chars = 12000;
    bool top_k_explicit = false;
    bool max_ctx_explicit = false;
    std::string profile = "balanced";
    bool force_reindex = false;
    std::optional<std::string> db_path_arg;
    std::vector<std::string> positional;

    for (int i = 1; i < argc; ++i) {
        std::string arg = argv[i];
        if (arg == "--emb-port") {
            if (i + 1 >= argc || !parse_port(argv[i + 1], emb_port)) {
                std::cerr << "Invalid value for --emb-port\n";
                return 1;
            }
            ++i;
            continue;
        }
        if (arg == "--chat-port") {
            if (i + 1 >= argc || !parse_port(argv[i + 1], chat_port)) {
                std::cerr << "Invalid value for --chat-port\n";
                return 1;
            }
            ++i;
            continue;
        }
        if (arg == "--db-path") {
            if (i + 1 >= argc) {
                std::cerr << "Invalid value for --db-path\n";
                return 1;
            }
            db_path_arg = argv[i + 1];
            ++i;
            continue;
        }
        if (arg == "--top-k") {
            if (i + 1 >= argc || !parse_size_value(argv[i + 1], top_k)) {
                std::cerr << "Invalid value for --top-k\n";
                return 1;
            }
            top_k_explicit = true;
            ++i;
            continue;
        }
        if (arg == "--max-context-chars") {
            if (i + 1 >= argc || !parse_size_value(argv[i + 1], max_context_chars)) {
                std::cerr << "Invalid value for --max-context-chars\n";
                return 1;
            }
            max_ctx_explicit = true;
            ++i;
            continue;
        }
        if (arg == "--profile") {
            if (i + 1 >= argc) {
                std::cerr << "Invalid value for --profile\n";
                return 1;
            }
            profile = argv[i + 1];
            if (profile != "fast" && profile != "balanced" && profile != "accurate") {
                std::cerr << "Invalid value for --profile (expected: fast|balanced|accurate)\n";
                return 1;
            }
            ++i;
            continue;
        }
        if (arg == "--reindex") {
            force_reindex = true;
            continue;
        }
        if (!arg.empty() && arg[0] == '-') {
            std::cerr << "Unknown argument: " << arg << "\n";
            return 1;
        }
        positional.push_back(arg);
    }

    if (positional.size() != 2) {
        std::cerr << "Usage: local_rag [--emb-port <port>] [--chat-port <port>] [--profile <fast|balanced|accurate>] [--top-k <k>] [--max-context-chars <n>] [--reindex] [--db-path <path>] <folder> <question>\n";
        return 1;
    }

    if (!top_k_explicit || !max_ctx_explicit) {
        size_t profile_top_k = 4;
        size_t profile_max_ctx = 12000;
        if (profile == "fast") {
            profile_top_k = 2;
            profile_max_ctx = 4000;
        } else if (profile == "accurate") {
            profile_top_k = 6;
            profile_max_ctx = 20000;
        }
        if (!top_k_explicit) top_k = profile_top_k;
        if (!max_ctx_explicit) max_context_chars = profile_max_ctx;
    }

    const std::string folder = positional[0];
    const std::string question = positional[1];

    const std::string emb_base = "http://127.0.0.1:" + std::to_string(emb_port);
    const std::string chat_base = "http://127.0.0.1:" + std::to_string(chat_port);

    constexpr size_t CHUNK_SIZE = 800;
    constexpr size_t CHUNK_OVERLAP = 120;

    fs::path db_path = db_path_arg ? fs::path(*db_path_arg) : (fs::path(folder) / ".local_rag.sqlite");

    sqlite3* db = nullptr;
    if (sqlite3_open(db_path.string().c_str(), &db) != SQLITE_OK) {
        std::cerr << "Failed to open SQLite DB: " << db_path << "\n";
        if (db) sqlite3_close(db);
        return 2;
    }

    sqlite_exec(db, "PRAGMA journal_mode=WAL;");
    sqlite_exec(db, "PRAGMA synchronous=NORMAL;");

    bool vec_loaded = try_load_sqlite_vec(db);
    if (!init_db(db)) {
        sqlite3_close(db);
        return 2;
    }

    if (force_reindex) {
        clear_chunks(db);
    }

    if (!sqlite_exec(db, "CREATE TEMP TABLE seen_chunks(chunk_id TEXT PRIMARY KEY);")) {
        sqlite3_close(db);
        return 2;
    }

    auto files = collect_doc_files(folder);
    size_t inserted_or_updated = 0;
    size_t skipped = 0;

    for (const auto& p : files) {
        std::string content = read_text_file(p);
        if (content.empty()) continue;

        std::uintmax_t sz = 0;
        try { sz = fs::file_size(p); } catch (...) {}
        std::int64_t mt = file_mtime_key(p);
        std::string source_sig = std::to_string(static_cast<unsigned long long>(sz)) + ":" + std::to_string(mt);

        auto chunks = chunk_text(content, CHUNK_SIZE, CHUNK_OVERLAP);
        int ci = 0;
        for (auto& ch : chunks) {
            std::string chunk_id = p.string() + "#" + std::to_string(ci++);
            mark_seen(db, chunk_id);

            auto old_sig = get_chunk_signature(db, chunk_id);
            if (old_sig && *old_sig == source_sig) {
                ++skipped;
                continue;
            }

            auto vec = embed(emb_base, ch);
            if (!vec) continue;
            if (!upsert_chunk(db, chunk_id, p.string(), ch, vec_to_json_text(*vec), source_sig)) {
                std::cerr << "Failed to upsert chunk: " << chunk_id << "\n";
                continue;
            }
            ++inserted_or_updated;
        }
    }

    prune_unseen(db);

    size_t indexed = count_chunks(db);
    std::cout << "Indexed " << indexed << " chunks"
              << " (updated " << inserted_or_updated << ", reused " << skipped << ")\n";

    auto qv = embed(emb_base, question);
    if (!qv) {
        std::cerr << "Failed to embed question\n";
        sqlite3_close(db);
        return 3;
    }

    std::vector<RetrievedChunk> hits;
    std::string qvec_json = vec_to_json_text(*qv);
    bool used_vec = false;
    size_t candidate_k = std::max<size_t>(top_k * 20, 50);
    if (vec_loaded) {
        hits = retrieve_with_vec(db, qvec_json, question, candidate_k, top_k, used_vec);
    }
    if (hits.empty()) {
        hits = retrieve_fallback(db, *qv, question, top_k);
        used_vec = false;
    }

    std::string context;
    context.reserve(max_context_chars + 256);
    for (auto& h : hits) {
        std::string block = "\n---\nFILE: " + h.path + "\n" + h.text + "\n";
        if (context.size() >= max_context_chars) break;
        size_t remaining = max_context_chars - context.size();
        if (block.size() <= remaining) context += block;
        else {
            context += block.substr(0, remaining);
            break;
        }
    }

    std::string system = "You are a precise assistant. Answer ONLY using the provided CONTEXT. "
                         "If the answer is not present, say you don't know and cite file names.";
    std::string user = "QUESTION:\n" + question + "\n\nCONTEXT:\n" + context + "\n\nAnswer with file citations.";

    auto ans = chat(chat_base, system, user);
    if (!ans) {
        std::cerr << "Generation failed\n";
        sqlite3_close(db);
        return 4;
    }

    std::cout << "Retrieval backend: " << (used_vec ? "sqlite_vec" : "fallback-cosine") << "\n";
    std::cout << "\n=== ANSWER ===\n" << *ans << "\n";

    sqlite3_close(db);
    return 0;
}
